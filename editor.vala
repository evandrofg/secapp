...
int secapp_work(int pid){
    ...
    try {
        ...
        int reply = sapp.requestPermissionFromPid (pid);
        stdout.printf ("%d\n", reply);
...
class MyWindow: Gtk.ApplicationWindow {
	...
	internal MyWindow (MyApplication app) {
		Object (application: app, title: "Text Editor");
	...
	void secapp_cb (SimpleAction action, Variant? parameter) {
		print ("Secapp called!\n");
		print ("dbg: %d\n", Posix.getpid());
		secapp_work(Posix.getpid());
	}
...
class MyApplication: Gtk.Application {
	...
	protected override void startup () {
		...
		try {
			builder.add_from_file ("/usr/share/editor.ui");
		} catch (GLib.Error err) {
